...
| `gitim-updater` | self-update 共享核心（CLI 和 runtime 共用） | 版本解析、平台检测、`fetch_latest_tag`、`install_update`、原子替换 `replace_binaries` |
...
**全量 `cargo test` 是昂贵操作**（700+ 测试，包含启动真实 daemon 的集成测试，耗时以分钟计）。验证应按改动面选择最小但充分的集合：

- **文档 / 注释 / 纯配置变更**：检查内容、链接和 `git diff --check`；没有 Rust 行为变化时不跑 Cargo 测试。
- **Frontend-only**：先跑相关 Vitest 文件（例如 `npm exec vitest -- run src/lib/foo.test.ts`）；涉及类型或打包边界时再加 `npm run lint`、`npm run build`。
- **单 crate Rust 变更**：跑对应 crate、`--test` target 或测试名过滤（`cargo test -p <crate>`、`cargo test -p <crate> --test <target>`、`cargo test <name_substring>`）。
- **跨 crate 但边界清晰**：先分别跑受影响 crate 的 scoped 测试，不因“跨了两个目录”直接升级全量。
- **Subagent / 并行任务**：各自只跑负责范围内的 scoped 测试，由主线在汇总后判断是否需要更大验证集。

**只有以下情况跑全量 `cargo test`**：

1. 用户明确要求（如“跑一下全量”“跑 cargo test”“全跑一次确认 regression”）。
2. 改动触及共享类型 / 协议、workspace 级依赖、build script，或影响范围无法由 scoped 测试可靠覆盖。

不要因为任务开始、任务结束或准备提交而惯性跑全量。提交 hook 已负责 `cargo fmt` 和 `cargo clippy`，不会替代行为测试，也不构成额外跑全量测试的理由。
