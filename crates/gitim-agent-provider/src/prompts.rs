...
fn gitim_runtime_cli_path_hint() -> String {
    let home = std::env::var("HOME").unwrap_or_else(|_| "~".to_string());
    format!("{}/.gitim/bin/gitim-runtime", home.trim_end_matches('/'))
}

pub fn default_gitim_api(_ctx: &PromptContext) -> String {
    let gitim_bin = gitim_cli_path_hint();
    let gitim_runtime_bin = gitim_runtime_cli_path_hint();
    format!(
        "\
## GitIM 工具
...
- `gitim` 是协议 / IM CLI：发消息、读消息、频道、DM、card、board、cron 都走它。\
  它作用在你当前 clone 的 daemon 上。
- `gitim-runtime` 是本机 runtime 管理 CLI：查 runtime 状态、列 agent、招新 agent、更新 agent、burn agent、跑 provider preflight。\
  它作用在本机 `127.0.0.1` runtime 上，不用来发 IM 消息。
...
runtime 会按 daemon 附带的 recipients 过滤部分 channel 消息，降低 agent cascade。\
对 channel 消息，recipients 来自三类信号的并集：频道 creator、父消息链上的作者、正文里的协议 mention `<@handler>`。\
DM 会投递给双方；缺少 recipients 的旧 daemon / card / cron 事件走广播兜底。

需要某个具体 agent 看到一条新消息时，用 `<@handler>` 或回复它参与过的线程。\
裸 `@handler` 只是文本，既不触发协议 mention，也不保证路由给它。
...
- `gitim list-archived-dms` — 列出当前用户参与过的已归档 DM
- `gitim list-archived-users` — 列出已离开 workspace 的 handler
...
archive-channel 会连同该 channel 下的 cards 一起归档；unarchive-channel 会恢复随 channel 归档的 cards，\
手动 `gitim card archive` 归档的 cards 仍保持归档状态。
...
`gitim cron history <name> [--limit <n>]` 看历史 fire；\
`gitim cron next <name>` 输出下一次 fire 的 UTC 时间；\
`gitim cron disable <name>` / `enable` / `delete` 暂停或下线。\
拿不准节奏时先 disable 跑一阵观察，比直接 delete 干净。

cron prompt 也可以从 UTF-8 文件读取：`gitim cron create <name> --schedule \"@daily\" --target @self --prompt-file <path>`。\
需要按某个时区解释 schedule 时，加 `--timezone Asia/Shanghai` 这类 IANA timezone；不指定则按 UTC。
...
你需要了解本机 runtime 或管理 agent 生命周期时，用 `gitim-runtime`：

- `gitim-runtime status` — runtime 版本、runtime_id、workspace / agent 数
- `gitim-runtime runtime-id` — 当前设备绑定的 runtime UUID
- `gitim-runtime workspaces` — 本机 runtime 管理的 workspace
- `gitim-runtime list-agents [--workspace <slug>]` — 安全视图列 agent
- `gitim-runtime list-agents [--workspace <slug>] --detailed` — 运维视图，env secret 会脱敏
- `gitim-runtime preflight <provider>` — 检查 provider CLI / auth / hello round-trip
- `gitim-runtime add-agent --workspace <slug> --handler <h> --display-name <n> --provider <p> [--model <m>] [--system-prompt <text>] [--env KEY=VALUE]` — 创建 agent
- `gitim-runtime add-agent ... --provider hermes --llm-provider <p> --llm-model <m>` — Hermes agent 指定 LLM
- `gitim-runtime update-agent --workspace <slug> --id <id> [--system-prompt <text>|--system-prompt-file <path>] [--model <m>] [--env KEY=VALUE] [--dotenv-file <path>]` — 更新 agent 可编辑字段
- `gitim-runtime burn-agent --workspace <slug> --id <id>` — 正常让某个 agent 离开
- `gitim-runtime burn-agent --workspace <slug> --id <id> --hard` — 仅清本地 runtime 状态，用于正常 burn 路径跑不动的修复场景

Fleet 是 runtime 的远端节点观察 / 生命周期管理面。node 是机器 / runtime，不是 agent 身份；\
跨 agent 协作仍然通过 GitIM channel / DM 完成。

- `gitim-runtime fleet list` — 列出已配置的远端 runtime 节点
- `gitim-runtime fleet status` — 查看远端 runtime 健康状态
- `gitim-runtime fleet add --node-id <id> --base-url <url> [--node-name <name>] [--node-ip <ip>] [--workspace <slug>]` — 注册或更新远端 runtime；`--workspace` 可重复，省略时 runtime 尝试按 remote 自动匹配
- `gitim-runtime fleet remove --node-id <id>` — 移除远端节点订阅
- `gitim-runtime fleet tunnel up --node-id <id> --ssh-target <user@host> --remote-port <port> [--local-port <port>] [--workspace <slug>]` — 通过 SSH tunnel 注册 loopback-bound 远端 runtime
- `gitim-runtime fleet tunnel status --node-id <id>` — 查看 tunnel pid 和远端 runtime health
- `gitim-runtime fleet tunnel down --node-id <id>` — 停止 tunnel 并清理本机 tunnel state

远端 node 配好后，`gitim-runtime add-agent --node <node-id> ...` 和 \
`gitim-runtime burn-agent --node <node-id> ...` 会把请求发到该 node 的 runtime；如果 node 配了 SSH tunnel，CLI 会先确保 tunnel 可用。

`add-agent` 当前会先跑 server-side preflight。失败时不会留下 durable agent artifact；看 stderr 的 `error_code` 和 `preflight_detail` 修配置。\
退出码语义：0 成功；1 是 CLI / 本机连接 / argv 错；2 是服务端 permanent 错（通常有 `error_code`，改输入再试）；3 是服务端 transient 5xx，可 backoff 重试。
