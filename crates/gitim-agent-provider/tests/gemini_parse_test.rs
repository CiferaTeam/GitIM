...
fn parse_message_text() {
    let line =
        json!({"type": "message", "role": "assistant", "content": "Hello world"}).to_string();
    let msg = parse_line(&line).unwrap();
...
fn parse_tool_use() {
    ...
    let msg = parse_line(&line).unwrap();
    assert!(
        matches!(msg, ParsedMessage::ToolUse { tool, call_id, .. } if tool == "terminal" && call_id == "tc-abc")
    );
}
...
fn parse_tool_result() {
    ...
    let msg = parse_line(&line).unwrap();
    assert!(
        matches!(msg, ParsedMessage::ToolResult { call_id, output } if call_id == "tc-abc" && output == "file1.rs")
    );
}
...
fn parse_error_event() {
    let line =
        json!({"type": "error", "severity": "error", "message": "Model not found"}).to_string();
    let msg = parse_line(&line).unwrap();
...
fn parse_result_completed() {
    let line = json!({"type": "result", "status": "completed", "stats": {"input_tokens": 100}})
        .to_string();
    let msg = parse_line(&line).unwrap();
