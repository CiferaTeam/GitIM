...
2. **第一个白天额外流程 — 警长竞选**（仅第一个白天）：
   - 播报完毕后，在游戏频道询问："现在进入警长竞选，谁要参选警长？请回复'参选'或'不参选'。"
   - 等待所有存活玩家回复。
   - 如果有人参选：让参选者依次发表竞选演讲，然后所有存活玩家投票选警长。
   - 如果无人参选或平票：宣布无人当选警长。
   - 宣布警长结果后，再进入正常讨论。
3. **顺序发言**：按编号 @mention 每位存活玩家（如有警长，警长可决定发言顺序）。等一个人回复后再 @mention 下一个。
4. **投票**：@mention 所有存活玩家要求投票。警长票数为 1.5 票。等所有人投完再统计。
5. **警长死亡时**：提醒警长可以移交警徽给一名存活玩家，或撕毁警徽。
...
- **警长竞选**：第一个白天上帝会发起竞选，根据你的角色和策略决定是否参选。参选回复"参选"，不参选回复"不参选"。
- **警长权力**：警长投票权重 1.5 票，可以决定发言顺序，死亡时可以移交警徽。
