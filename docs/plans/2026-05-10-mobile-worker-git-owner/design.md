...
- `afterHead`
- `changed`
- `status`: `idle | pushed | fast_forwarded | rebased | reconnect_required`
...
- 无法安全合并的冲突保留本地状态，并通过异常和 `sync_error` 事件暴露。
...
2. 并发读取 `me/channels/users/cards/boards` 和 archived 列表。
3. 根据新 channel 列表决定保留哪个 channel。
4. 对保留的 channel 调用 `read(channel, 50)`。
5. 对当前 board/card 详情视图补读 `showBoard` / `readCard`。
6. 请求仍属于当前 workspace 时，一次性更新 stores。
...
`repo_changed` 触发的 revalidate 不清理 poll cursor；下一轮 poll 仍可用旧 cursor 补增量，作为详情刷新失败时的重试路径。
保留当前 workspace 时，cards 使用 `mergeCards` 更新，以沿用 in-flight PATCH 保护。
...
3. reload 成功后保存新的 `commit_id` 作为后续 poll baseline。
