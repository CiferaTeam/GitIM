...
| `docs/protocol-typing.md` | 创建 | 协议契约规则（守门用） |
| `CLAUDE.md` | 修改 | 加一条"新 endpoint 必须 typed Response"标准 |
| `.github/workflows/lint.yml` 或 `scripts/check-protocol-typing.sh` | 创建（可选） | grep guard 阻止新增 `Json(json!(` |
...
**Files:**
- Create: `docs/protocol-typing.md`
- Modify: `CLAUDE.md`（在"约定"或"测试"段落附近加一条）

- [ ] **Step 1: 写 `docs/protocol-typing.md`**

内容覆盖：
- 协议层定义：哪些代码路径属于"协议边界"（daemon IPC、runtime HTTP、SSE event、me.json 落盘）
- 类型化要求：边界两端必须有对应 struct/enum；handler 不得用 `Json(json!())` 构造成功响应（错误体豁免）
- 豁免清单：LLM 输出、第三方 API 透传、用户扩展字段、错误响应字符串
- 新增 endpoint / Request 变体的 checklist：`XxxRequest` struct + `XxxResponse` struct + 序列化 snapshot 测试
- 协议变更流程：改字段名等同破坏性变更，需要在 PR 描述中标注"protocol-break"

- [ ] **Step 2: 在 CLAUDE.md 加一条标准**

在 "## 约定" 或新增 "## 协议层" 段落，加一行指向 `docs/protocol-typing.md` 的 reference + 概要规则（"新 endpoint 必须 typed Response，禁止 `Json(json!())` 构造成功响应"）。

- [ ] **Step 3: Commit**

`git add docs/protocol-typing.md CLAUDE.md && git commit -m "docs: protocol typing contract"`

**验收**：新人/AI 接手时能从 CLAUDE.md 一跳到契约文档；契约文档列举完豁免和必做项。
...
记录"协议类型化已落地：daemon Response payload 已 typed（阶段性完成 N/total）；Onboard auth 是 AuthPayload enum；runtime endpoint 响应走 typed Response struct + ErrorBody；新 endpoint 必须 typed。"
