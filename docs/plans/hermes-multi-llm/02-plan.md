...
## Baseline Verification 2026-05-10

> Phase 0 results. All commands run in worktree `/Users/lewisliu/ateam/GitIM/.claude/worktrees/laughing-curie-aac17c/`. Hermes config backup/restore confirmed.

### V1: `hermes config set` dotted-path support

**GREEN.** All three dotted paths write correctly to nested YAML, preserving sibling keys:

```
hermes config set model.provider zai     → model: {provider: zai, default: MiniMax-M2.7-highspeed, base_url: ...}  ✓
hermes config set model.default foo      → model: {provider: zai, default: foo, base_url: ...}                      ✓
hermes config set model.base_url <url>   → model: {provider: zai, default: foo, base_url: <url>}                    ✓
```

No clobber of sibling fields. Plan's shell-out-three-times approach works exactly as expected.

### V2: Builtin provider registry cross-check (`auth.py` PROVIDER_REGISTRY)

Source: `~/ateam/code-skills/.repos/hermes-agent/hermes_cli/auth.py` lines 99–236.

| Provider | env_vars (spec → actual) | base_url (spec → actual) | Status |
|----------|--------------------------|--------------------------|--------|
| anthropic | ANTHROPIC_API_KEY, ANTHROPIC_TOKEN, CLAUDE_CODE_OAUTH_TOKEN | https://api.anthropic.com | **GREEN** exact match |
| deepseek | DEEPSEEK_API_KEY | https://api.deepseek.com/v1 | **GREEN** exact match |
| kimi-coding | KIMI_API_KEY | https://api.moonshot.ai/v1 | **YELLOW — partial match** (see note) |
| minimax | MINIMAX_API_KEY | https://api.minimax.io/anthropic | **GREEN** exact match |
| minimax-cn | MINIMAX_CN_API_KEY | https://api.minimaxi.com/anthropic | **GREEN** exact match |
| zai | GLM_API_KEY, ZAI_API_KEY, Z_AI_API_KEY | https://api.z.ai/api/paas/v4 | **GREEN** exact match |

**kimi-coding note (YELLOW):** `auth.py` has a `_resolve_kimi_base_url()` function that auto-detects key prefix at runtime: if `KIMI_API_KEY` starts with `sk-kimi-`, the actual base_url routes to `https://api.kimi.com/coding/v1` instead of the registry default `https://api.moonshot.ai/v1`. The user's current `KIMI_API_KEY` is `sk-kimi-*` prefix — so `base_url` that `introspect.rs` reads from the registry will be wrong for this user. The plan's `introspect.rs` needs to replicate this prefix detection logic or read the resolved URL from hermes, not the static registry entry.

### V3: `/models` endpoint probing

| Provider | Key available | Probe result |
|----------|---------------|--------------|
| anthropic | NO | Skipped. Auth error (401) confirms endpoint exists at `/v1/models`; assumed OpenAI-compatible at implementation |
| deepseek | NO | Skipped. No key — assume OpenAI-compatible `/models` at implementation |
| kimi-coding | YES (sk-kimi-*) | `GET https://api.kimi.com/coding/v1/models` → **200 OK**, OpenAI list format. `GET https://api.moonshot.ai/v1/models` → 401 (auth required, not 404 — endpoint exists) |
| minimax | NO | `GET https://api.minimax.io/anthropic/models` → **404**. Anthropic-protocol provider — no `/models` list endpoint at its base_url path. Needs special handling |
| minimax-cn | YES | `GET https://api.minimaxi.com/anthropic/models` → **404**. `GET https://api.minimaxi.com/v1/models` → 200 but `{"object":"","data":null}` (empty, not useful). Anthropic-protocol provider — no usable `/models` |
| zai | NO | Skipped. No key — assume OpenAI-compatible `/models` at implementation |

**Critical finding for implementation:** Both `minimax` and `minimax-cn` use an Anthropic Messages protocol base_url (`/anthropic` suffix), not OpenAI-compatible. Their `/models` path returns 404. The plan's `fetch_models` in `models.rs` must handle these as Anthropic-protocol providers that cannot list models — return empty list + static fallback, or use a different models path. `BuiltinProvider` needs a `protocol: ApiProtocol` field (or at minimum an `api_mode` field) to distinguish OpenAI-compatible from Anthropic-compatible, which affects both `/models` fetch and (eventually) the hermes config that the profile receives.

### V4: cargo test baseline

**`cargo test --workspace` result:** 913 passed, 0 failed, 16 ignored

All ignored tests are legitimately gated (codex rollout fixture, claude/agent_loop real-CLI tests). Zero red tests. Clean baseline confirmed.

### V5: hermes binary status

```
Hermes Agent v0.10.0 (2026.4.16)
```

Profiles: 3 total (`default`, `gitim-akak2`, `gitim-hermes-smoke-test`). Binary on PATH. Note: 764 commits behind — `hermes update` available but not run (not needed for this task).

---

### Summary: Plan Assumptions vs Reality

| Assumption | Status | Action needed |
|------------|--------|---------------|
| `hermes config set` writes nested YAML | **CONFIRMED** | None |
| 6 builtin providers env_vars/base_url match | **5 of 6 confirmed** (kimi partial) | `introspect.rs` must replicate `_resolve_kimi_base_url()` prefix detection |
| All providers expose `/models` at base_url | **PARTIALLY BROKEN** | `minimax` + `minimax-cn` are Anthropic-protocol — no `/models`. Need `ApiProtocol` enum in `BuiltinProvider` |

**No plan redesign needed.** The kimi URL resolution and minimax protocol distinction are implementable adjustments within the current architecture. `BuiltinProvider` struct should gain `api_protocol: ApiProtocol` (OpenAI vs Anthropic) and `kimi-coding` entry needs runtime URL resolution logic mirroring hermes `_resolve_kimi_base_url`.
...
- `registry.rs`:
  - `pub enum ApiProtocol { OpenAI, Anthropic }` —— Phase 0 V3 发现 minimax 系列没有 `/models` endpoint(Anthropic-protocol),fetch_models 必须按 protocol 短路
  - `pub struct BuiltinProvider { pub id: &'static str, pub label: &'static str, pub env_vars: &'static [&'static str], pub base_url: &'static str, pub api_protocol: ApiProtocol }`
  - `pub const BUILTIN_PROVIDERS: &[BuiltinProvider]` 6 项,顺序 alphabetic by id;`api_protocol` 值跟 spec Section "BUILTIN_PROVIDERS" 表一致(`minimax` / `minimax-cn` = Anthropic,其余 = OpenAI)
  - 顶部 `// Mirrored from hermes_cli/auth.py:PROVIDER_REGISTRY @ v0.10.0 (2026.4.16, verified Phase 0). Resync on hermes minor bumps; CI does not enforce.` 注释
  - **kimi-coding base_url** 字段填 `"https://api.moonshot.ai/v1"` (Moonshot 默认 fallback,Phase 0 V2 确认)。**真实使用前** introspect 模块根据 `KIMI_API_KEY` 前缀决定是否 override 为 `"https://api.kimi.com/coding/v1"`(`sk-kimi-*` 前缀走 coding endpoint;其他用户保持 default)。registry 不动,运行时 resolve
...
6. `registry_minimax_protocols_anthropic` —— assert id="minimax" 和 id="minimax-cn" 的 `api_protocol == ApiProtocol::Anthropic`
7. `registry_others_protocol_openai` —— assert id="anthropic"/"deepseek"/"kimi-coding"/"zai" 的 `api_protocol == ApiProtocol::OpenAI`(注:provider id "anthropic" 是 OpenAI-protocol —— 走 Anthropic 公司的 OpenAI-compatible API 路径)
...
- 实现:
  ...
  2. 对 `BUILTIN_PROVIDERS` 每个 entry,检查它的 `env_vars` 任一在 .env 里出现且值非空 → 决定 base_url 的最终值:
     - 默认用 registry 里 `entry.base_url`
     - **特例:`kimi-coding`** —— 命中的 env_var 是 `KIMI_API_KEY` 时,读其值,前缀以 `sk-kimi-` 开头则 override 为 `"https://api.kimi.com/coding/v1"`(mirror `auth.py:_resolve_kimi_base_url`),否则保留默认 `api.moonshot.ai/v1`。把这段抽成 helper `pub(crate) fn resolve_builtin_base_url(provider: &BuiltinProvider, env_value: &str) -> String` 方便单元测
     - 然后 push 一条 `{ id, label, kind: ApiKey, base_url: Some(<resolved>), api_protocol: <from registry> }`
  3. 读 `<hermes_home>/config.yaml`,parse YAML 取 `custom_providers: List[{name, base_url, ...}]`,每条 push `{ id: format!("custom:{name}"), label: format!("{name} (custom)"), kind: Custom, base_url: Some(<from entry>), api_protocol: ApiProtocol::OpenAI }`(custom 默认假定 OpenAI 协议)
...
- **`LlmProvider` 结构** 加一个 `pub api_protocol: ApiProtocol` 字段(reexport 自 registry),方便下游 fetch_models 直接据此决策,不用回查 registry

**测试(`tests/hermes_llm_introspect.rs`):**
1. `empty_hermes_home_returns_empty_list` —— hermes_home = 不存在的 tempdir 子路径
2. `env_with_key_lists_provider` —— 写 `KIMI_API_KEY=mk_xxx` 到 tempdir/.env(非 sk-kimi-* 前缀),断言 list 含 id=kimi-coding 且 base_url == `https://api.moonshot.ai/v1`
...
5. `config_yaml_custom_providers_listed` —— 写 yaml 含 `custom_providers: [{name: my-glm, base_url: https://x}]`,断言 list 含 id=`custom:my-glm`,kind=Custom,api_protocol=OpenAI
...
9. `kimi_with_sk_kimi_prefix_resolves_coding_url` —— 写 `KIMI_API_KEY=sk-kimi-abc123`,断言 list 里 id=kimi-coding 的 base_url == `https://api.kimi.com/coding/v1`
10. `kimi_with_other_prefix_keeps_moonshot_url` —— 写 `KIMI_API_KEY=mk-abc123`,断言 base_url == `https://api.moonshot.ai/v1`(default fallback)
11. `minimax_protocol_propagates_to_listprovider` —— 写 `MINIMAX_API_KEY=foo`,断言返回的 LlmProvider.api_protocol == Anthropic

**验收标准:**
- 11 个集成测试全绿
- `cargo clippy` 无新 warning
- `list_providers` 是纯函数(除了 fs 读),无 side effect
- `resolve_builtin_base_url` helper 暴露为 `pub(crate)`,供单测直接断言(如有需要可加 inline unit test)
...
- `pub async fn fetch_models(provider: &LlmProvider, hermes_home: &Path) -> ModelListResult`
  1. **Anthropic-protocol short-circuit:** 若 `provider.api_protocol == ApiProtocol::Anthropic`,**不打网络**,直接返回 `ModelListResult { models: vec![], custom_allowed: true, error: Some(format!("{} uses Anthropic protocol; /models not supported. Use Custom...", provider.id)), fetched_at_ms: now }`。Phase 0 V3 verified `minimax` / `minimax-cn` 真的没 `/models` endpoint
  2. 解析 base_url(provider.base_url 缺 → error="missing base_url for provider <id>")
  3. 解析 API key:
  ...
  5. GET `<base_url>/models`,header `Authorization: Bearer <key>`(OpenAI 兼容假设;custom provider 同)
  6. 错误分类(spec 失败模式表):
...
9. `anthropic_protocol_short_circuits_without_network` —— 构造 LlmProvider { api_protocol: Anthropic, base_url: Some("http://127.0.0.1:9999/should-never-hit") } —— 9999 端口不应该被 mock 监听;assert 函数返 200 + error 含 "Anthropic protocol",且 httpmock 无任何 hit(用 `mock.hits() == 0` 断言)。这是 spec 的"Anthropic-protocol 永远 fallback Custom"的关键测试
