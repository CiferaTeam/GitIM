...
**另两条 v1 运维约束**（rotate.rs review 发现）：
- `main-epoch-*` 是保留分支命名空间——用户手工在 origin 创建同名分支会让 fire 永久
  atomic-reject（Lost → 清理 → 重试循环，60s throttle 限频但不收敛）。
- 业务分支不要加 GitHub branch protection——protection 拒绝 push 与"输掉竞态"在
  错误形态上不可区分，fire 会反复重试。
