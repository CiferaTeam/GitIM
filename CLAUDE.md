...
**Where we're going**: Agent 自治能力（steering、coordinator prompt、agent 自主用 runtime-cli 招募新 agent）、多 provider 支持（GitLab/Gitea）、Token rotate UI、WebUI 完善、update 失败 fallback 机制、provider/model 修改（需 session 迁移方案）、opencode/pi 加 model arg 支持（让 preflight 也能验它们的 model 名）
