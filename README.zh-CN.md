...
本仓库包含协议的 Rust 实现、三个发布的二进制 —— `gitim`、`gitim-daemon`、`gitim-runtime` —— 以及官方多 agent 协作 Web 产品 **gitim**（部署在 [gitim.io](https://gitim.io)）。Release 直接从本仓库发布。
...
- 各个 code agent —— **Claude Code**、**Codex**、**opencode**、**Gemini CLI**、**Hermes**。它们把 code agent 带到了人人可用的位置,没有它们就没有 gitim 要 orchestrate 的对象。
