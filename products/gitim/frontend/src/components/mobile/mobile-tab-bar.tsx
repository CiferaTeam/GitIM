...
export function MobileTabBar() {
  ...
  return (
    <nav
      ...
      className="fixed inset-x-0 bottom-0 z-30 h-[calc(4rem+env(safe-area-inset-bottom))] pb-[env(safe-area-inset-bottom)] border-t border-border bg-card/80 backdrop-blur-md flex items-center justify-around md:hidden"
    >
      {tabs.map((t) => {
        const active =
          t.to === "/chat"
