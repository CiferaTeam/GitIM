...
app.post("/api/verify", async (c) => {
  ...
  const raw2 = await c.env.CELL_KV.get(kvKey(code));
  if (!raw2) {
...
// src/heartbeat.ts
var app2 = new Hono2();
app2.post("/api/heartbeat", async (c) => {
  const body = await c.req.json();
  const code = body.code?.trim();
  const deviceId = body.device_id?.trim();
  if (!code || !deviceId) {
  ...
  device.last_seen = (/* @__PURE__ */ new Date()).toISOString();
  await c.env.CELL_KV.put(kvKey(code), JSON.stringify(invite));
...
app3.get("/admin/codes", async (c) => {
  const list = await c.env.CELL_KV.list({ prefix: "invite:" });
  const codes = [];
  for (const key of list.keys) {
    const raw2 = await c.env.CELL_KV.get(key.name);
    if (raw2) codes.push(JSON.parse(raw2));
  }
  return c.json({ codes });
});
app3.post("/admin/codes", async (c) => {
  const body = await c.req.json();
  const code = body.code?.trim();
...
app3.get("/admin/codes/:code", async (c) => {
  const code = c.req.param("code");
  const raw2 = await c.env.CELL_KV.get(kvKey(code));
  if (!raw2) return c.json({ error: "not found" }, 404);
  return c.json(JSON.parse(raw2));
});
app3.delete("/admin/codes/:code", async (c) => {
  const code = c.req.param("code");
  await c.env.CELL_KV.delete(kvKey(code));
  return c.json({ ok: true });
});
app3.delete("/admin/codes/:code/devices/:deviceId", async (c) => {
  const code = c.req.param("code");
...
app4.route("/", app2);
app4.route("/", app3);
app4.get("/", (c) => c.json({ service: "cell-api", status: "ok" }));
...
// .wrangler/tmp/bundle-ZSq0ZN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ZSq0ZN/middleware-loader.entry.ts
